...
export default function MenuCategorySlider() {
    ...
    return (
        <section className="w-full bg-white py-6 px-2 md:px-5 lg:px-6">

            {/* Top Row — Categories left, View Menu right */}
            <div className="flex items-center justify-between mb-6 overflow-x-auto scrollbar-hide px-1">
                <div className="flex items-center gap-4 md:gap-6 flex-nowrap ">
                    {categories.map((cat) => (
                        <button
                            key={cat}
                            onClick={() => setActiveCategory(cat)}
                            className="font-compact-black text-lg md:text-xl whitespace-nowrap transition-colors scrollbar-hide"
                ...
                <a
                    ...
                    className="font-compact-black cursor-pointer text-md md:text-xl tracking-normal whitespace-nowrap ml-6 transition-opacity hover:opacity-70"
                    style={{ color: "#00B4C5" }}
                >
                    VIEW OUR MENU
                </a>
